...
const requests_history = function(req, reply) {
    ...
    var friendsReceivedNeedProperties = {};
    var friendsReceivedNeedPropertiesArray = new Array();
    var responseObject = {};
    var count;
    var x=0;
    ...
    function callback3 (result){
        ...
        for(var x=0; x<result.records.length; x++){
            ...
            friendsReceivedNeedProperties = {
                "userid":result.records[x]._fields[4],
                "createtime":result.records[x]._fields[5],
                "needid":result.records[x]._fields[6],
                "request_id":result.records[x]._fields[7],
                "receiverid":result.records[x]._fields[8],
                "count":result.records[x]._fields[9].low
            };
            friendsHasNeedPropertiesArray[x] = friendsHasNeedProperties;
            friendsReceivedNeedPropertiesArray[x] = friendsReceivedNeedProperties;
        }

        responseObject.friendsHasNeed = friendsHasNeedPropertiesArray;
        responseObject.friendsReceivedNeed = friendsReceivedNeedPropertiesArray;
    ...
    function callback2 (result){
        ...
        var CQ_RH3 = "MATCH (u1:User {uid:'"+ req.payload.uid +"'})-[:FRIEND]->(u2), " +
            ...
            "WITH u1,u2,r1 " +
            "MATCH (u2)-[:RECEIVED]->(r2 {needid:r1.needid}), (u1)-[:PLEASE]->(r3 {needid:r1.needid}) " +
            "RETURN u2.uid, u2.mobile, u2.name, u2.image_url, r3.userid, r3.createtime, r3.needid, r3.request_id, r3.receiverid, r3.count";
        session
            .run(CQ_RH3)
            .then(callback3)
...
/* curl로 post 파라미터 보내기~
 curl -X POST -H "Content-Type: application/json" -d '{"uid":"7bf99ea1-5da6-4204-b2e1-7d9b9808f71c","request_id":"4261e4fe-ded3-4dff-bab5-06ac57562dc6"}' -i http://192.168.0.11:3000/requests/history
 */