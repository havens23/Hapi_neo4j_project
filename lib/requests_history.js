...
const requests_history = function(req, reply) {
    ...
    function callback1 (result){
        ...
        if(count==1){
            ...
            collaboHistoryArray[0]=collaboHistory;
        ...
        // 2 depth
            ...
            var CQ_RH2= "MATCH ()-[:RECEIVED]->(a)<-[:PLEASE]-()-[:RECEIVED]->(b)<-[:PLEASE]-()-[:PUBLISHED]->() " +
                "WHERE a.receiverid = '"+ req.payload.uid +"' " +
        ...
        // 3 depth
            ...
            var CQ_RH2= "MATCH ()-[:RECEIVED]->(a)<-[:PLEASE]-()-[:RECEIVED]->(b)<-[:PLEASE]-()-[:RECEIVED]->(c)<-[:PLEASE]-()-[:PUBLISHED]->() " +
                "WHERE a.receiverid = '"+ req.payload.uid +"'" +
        ...
        // 4 depth
            ...
            var CQ_RH2= "MATCH ()-[:RECEIVED]->(a)<-[:PLEASE]-()-[:RECEIVED]->(b)<-[:PLEASE]-()-[:RECEIVED]->(c)<-[:PLEASE]-()-[:RECEIVED]->(d)<-[:PLEASE]-()-[:PUBLISHED]->() " +
                "WHERE a.receiverid = '"+ req.payload.uid +"' " +
...
/* curl로 post 파라미터 보내기~
 curl -X POST -H "Content-Type: application/json" -d '{"uid":"9291c218-8b30-4462-a2d5-992928ad3f3f","request_id":"e9a27114-57aa-48b5-a6a1-ca9c34385240"}' -i http://192.168.0.11:3000/requests/history
 */